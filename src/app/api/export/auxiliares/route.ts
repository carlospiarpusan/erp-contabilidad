...
import { getAuxiliaresRows } from '@/lib/db/compliance'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const codigoCuenta = searchParams.get('codigo_cuenta') ?? undefined
    const format = resolveExportFormat(searchParams.get('format'))
    const rawRows = await getAuxiliaresRows({ desde, hasta, codigoCuenta })
    ...
    return createExportResponse({
      format,
      baseFilename: `auxiliares-${new Date().toISOString().split('T')[0]}`,
