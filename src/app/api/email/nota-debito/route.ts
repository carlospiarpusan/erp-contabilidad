...
import { getResend, emailFrom, htmlNotaDebito } from '@/lib/email'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!doc) return NextResponse.json({ error: 'Nota débito no encontrada' }, { status: 404 })
    ...
    const { error } = await resend.emails.send({
      from: emailFrom,
      to: [destino],
      subject: `Nota Débito ${doc.prefijo ?? ''}${doc.numero} — ${empresa?.nombre ?? ''}`,
      html: htmlNotaDebito({
        ...
        link:     `${appUrl}/print/nota-debito/${id}`,
      }),
    })
