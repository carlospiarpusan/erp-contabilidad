...
import { getPedidos, getResumenPedidos } from '@/lib/db/pedidos'
...
export default async function InformePedidosPage({ searchParams }: PageProps) {
  ...
  const [{ pedidos, total }, resumen] = await Promise.all([
    getPedidos({ desde, hasta, estado: estado || undefined, limit: PAGE_SIZE, offset }),
    getResumenPedidos({ desde, hasta, estado: estado || undefined }),
  ])
  ...
  function buildHref(nextPage: number) {
    ...
    return query ? `/informes/pedidos?${query}` : '/informes/pedidos'
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total pedidos', val: resumen.total.toString(), color: 'text-gray-900' },
          { label: 'Valor total', val: formatCOP(resumen.total_valor), color: 'text-blue-700', mono: true },
          { label: 'Aprobados', val: resumen.aprobados.toString(), color: 'text-green-700' },
          { label: 'Valor aprobados', val: formatCOP(resumen.valor_aprobados), color: 'text-green-700', mono: true },
        ].map(k => (
