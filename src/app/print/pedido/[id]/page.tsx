...
export default async function PrintPedidoPage({ params }: PageProps) {
  ...
  const lineas = ((pedido as never).lineas ?? []) as {
  ...
  const cliente = (pedido as never).cliente as {
    razon_social?: string; numero_documento?: string
    tipo_documento?: string; email?: string; telefono?: string
  } | null

  const bodega = (pedido as never).bodega as { nombre?: string } | null
        ...
        <div className="flex justify-between items-start mb-8 pb-6 border-b-2 border-gray-800">
          ...
          <div className="text-right">
            <div className="border-2 border-gray-800 px-4 py-2 rounded">
              ...
              <p className="text-2xl font-bold text-gray-900 font-mono">{(pedido as never).prefijo}{(pedido as never).numero}</p>
            </div>
            <p className="text-sm text-gray-600 mt-2">Fecha: <strong>{formatFecha((pedido as never).fecha)}</strong></p>
            {(pedido as never).fecha_vencimiento && (
              <p className="text-sm text-gray-600">Entrega: <strong>{formatFecha((pedido as never).fecha_vencimiento)}</strong></p>
            )}
            <p className="text-xs text-gray-400 mt-1">
              Estado: {ESTADO_LABEL[(pedido as never).estado] ?? (pedido as never).estado}
            </p>
        ...
        <div className="flex justify-end mb-8">
          <div className="w-64">
            <div className="flex justify-between text-sm py-1 border-b border-gray-100">
              ...
              <span className="font-mono">{formatCOP((pedido as never).subtotal)}</span>
            </div>
            {(pedido as never).total_descuento > 0 && (
              <div className="flex justify-between text-sm py-1 border-b border-gray-100">
                <span className="text-gray-600">Descuento</span>
                <span className="font-mono text-red-600">-{formatCOP((pedido as never).total_descuento)}</span>
              </div>
            )}
            <div className="flex justify-between text-sm py-1 border-b border-gray-100">
              <span className="text-gray-600">IVA</span>
              <span className="font-mono">{formatCOP((pedido as never).total_iva)}</span>
            </div>
            <div className="flex justify-between text-base font-bold py-2 border-t-2 border-gray-800 mt-1">
              <span>TOTAL</span>
              <span className="font-mono">{formatCOP((pedido as never).total)}</span>
            </div>
          </div>
        </div>

        {(pedido as never).observaciones && (
          <div className="mb-6 text-sm">
            <p className="font-semibold text-gray-700 mb-1">Observaciones:</p>
            <p className="text-gray-500 italic">{(pedido as never).observaciones}</p>
          </div>
        )}
