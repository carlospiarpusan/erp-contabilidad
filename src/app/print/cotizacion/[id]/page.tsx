...
export default async function PrintCotizacionPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="print:hidden flex items-center gap-3 px-6 py-3 bg-gray-100 border-b border-gray-200">
        <PrintButton />
        <a href={`/ventas/cotizaciones/${id}`} className="text-sm text-gray-500 hover:text-gray-700">← Volver</a>
      </div>
        ...
        <div className="flex justify-between items-start mb-8 pb-6 border-b-2 border-gray-800">
          ...
          <div className="text-right">
            ...
            {doc.fecha_vencimiento && (
              <p className="text-sm text-gray-600">Válida hasta: <strong>{formatFecha(doc.fecha_vencimiento)}</strong></p>
            )}
            <p className="text-xs text-gray-400 mt-1 capitalize">Estado: {doc.estado}</p>
          </div>
        </div>
