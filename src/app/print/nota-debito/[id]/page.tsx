...
export default async function PrintNotaDebitoPage({ params }: PageProps) {
  const { id } = await params
  const supabase = await createClient()

  const [{ data: nd }, empresa] = await Promise.all([
    ...
    `).eq('id', id).eq('tipo', 'nota_debito').single(),
    getEmpresa(),
  ])

  if (!nd) notFound()

  const cliente = nd.cliente as { razon_social?: string; numero_documento?: string; tipo_documento?: string; email?: string; telefono?: string; direccion?: string } | null
  const origen  = nd.factura_origen as { numero?: number; prefijo?: string; fecha?: string } | null
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="print:hidden flex items-center gap-3 px-6 py-3 bg-gray-100 border-b border-gray-200">
        <PrintButton />
        <a href={`/ventas/notas-debito/${id}`} className="text-sm text-gray-500 hover:text-gray-700">← Volver</a>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-8 py-8 print:px-6 print:py-4 print:max-w-none">
        ...
        <div className="flex justify-between items-start mb-8 pb-6 border-b-2 border-gray-800">
          ...
          <div className="text-right border-2 border-gray-800 px-5 py-3 rounded">
            <p className="text-xs font-bold text-gray-600 uppercase tracking-wide">Nota Débito</p>
            <p className="text-2xl font-bold text-gray-900 font-mono mt-1">{nd.prefijo}{nd.numero}</p>
            <p className="text-xs text-gray-500 mt-1">{formatFecha(nd.fecha)}</p>
          </div>
        </div>

        {/* Factura origen */}
        {origen && (
          <div className="mb-4 rounded bg-amber-50 border border-amber-200 px-4 py-2 text-sm">
            <span className="font-medium text-gray-700">Nota débito a factura: </span>
        ...
        {/* Motivo */}
        {nd.motivo && (
          <div className="mb-6 rounded bg-amber-50 border border-amber-200 px-4 py-2 text-sm text-amber-700">
            <span className="font-medium">Motivo: </span>{nd.motivo}
          </div>
        )}
        ...
        <table className="w-full text-sm mb-6 border-collapse">
          <thead>
            <tr className="border-b-2 border-gray-800">
              ...
              <th className="text-right py-2 px-2 text-gray-700 font-semibold">IVA%</th>
          ...
          <tbody>
            {lineas.map((l: any) => (
              <tr key={l.id} className="border-b border-gray-100">
                <td className="py-2 pr-4 text-gray-900">{l.descripcion}</td>
                ...
                <td className="text-right py-2 px-2 text-gray-500">{l.impuesto?.porcentaje ?? 0}%</td>
        ...
        {nd.observaciones && (
          <p className="text-xs text-gray-500 border-t border-gray-200 pt-4">{nd.observaciones}</p>
        )}
