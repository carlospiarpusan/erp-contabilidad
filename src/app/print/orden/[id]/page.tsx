...
type DocumentoImpresion = {
  ...
  proveedor?: ProveedorImpresion | null
...
export default async function PrintOrdenPage({ params }: PageProps) {
  const { id } = await params
  const [orden, empresa] = await Promise.all([
    getOrdenCompraById(id).catch(() => null),
    getEmpresa(),
  ])

  if (!orden) notFound()

  const doc = orden as DocumentoImpresion
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="print:hidden flex items-center gap-3 px-6 py-3 bg-gray-100 border-b border-gray-200">
        <PrintButton />
        <a href={`/compras/ordenes/${id}`} className="text-sm text-gray-500 hover:text-gray-700">← Volver</a>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-8 py-8 print:px-6 print:py-4 print:max-w-none">
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          ...
          {proveedor?.telefono && <p className="text-sm text-gray-600">Tel: {proveedor.telefono}</p>}
