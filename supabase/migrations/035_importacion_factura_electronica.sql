-- ============================================================
-- MIGRACION 035 - IMPORTACION DE FACTURA ELECTRONICA
-- - Equivalencias proveedor-producto para referencias externas
-- - GTIN unico por empresa en productos
-- - Validaciones de fecha/duplicado en factura_compra
-- - RPC atomica para importacion de factura electronica
-- ============================================================

CREATE TABLE IF NOT EXISTS public.productos_codigos_proveedor (
  ...
  CONSTRAINT productos_codigos_proveedor_referencia_chk
    CHECK (
      NULLIF(BTRIM(COALESCE(codigo_proveedor, '')), '') IS NOT NULL
      OR NULLIF(BTRIM(COALESCE(gtin, '')), '') IS NOT NULL
    )
);

CREATE INDEX IF NOT EXISTS idx_productos_codigos_proveedor_producto
  ON public.productos_codigos_proveedor(producto_id);

CREATE INDEX IF NOT EXISTS idx_productos_codigos_proveedor_proveedor
  ON public.productos_codigos_proveedor(empresa_id, proveedor_id);

CREATE UNIQUE INDEX IF NOT EXISTS idx_productos_codigo_barras_empresa_unico
  ON public.productos(empresa_id, codigo_barras)
  WHERE codigo_barras IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_productos_codigos_proveedor_gtin_unico
  ON public.productos_codigos_proveedor(empresa_id, proveedor_id, gtin)
  WHERE gtin IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_productos_codigos_proveedor_codigo_unico
  ON public.productos_codigos_proveedor(empresa_id, proveedor_id, codigo_proveedor)
  WHERE gtin IS NULL AND codigo_proveedor IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_documentos_factura_compra_numero_externo_unico
  ON public.documentos(empresa_id, proveedor_id, lower(btrim(numero_externo)))
  WHERE tipo = 'factura_compra'
    AND proveedor_id IS NOT NULL
    AND numero_externo IS NOT NULL
    AND btrim(numero_externo) <> '';

ALTER TABLE public.productos_codigos_proveedor ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "ver productos_codigos_proveedor" ON public.productos_codigos_proveedor;
CREATE POLICY "ver productos_codigos_proveedor" ON public.productos_codigos_proveedor
  FOR SELECT
  USING (
    empresa_id = public.get_empresa_id()
    AND public.get_rol_nombre() IN ('admin', 'contador')
  );

DROP POLICY IF EXISTS "gestionar productos_codigos_proveedor" ON public.productos_codigos_proveedor;
CREATE POLICY "gestionar productos_codigos_proveedor" ON public.productos_codigos_proveedor
  FOR ALL
  USING (
    empresa_id = public.get_empresa_id()
    AND public.get_rol_nombre() IN ('admin', 'contador')
...
CREATE OR REPLACE FUNCTION public.upsert_producto_codigo_proveedor(
  p_empresa_id uuid,
  p_proveedor_id uuid,
  p_producto_id uuid,
  p_codigo_proveedor text,
  p_gtin text,
  p_descripcion_proveedor text
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_codigo text := NULLIF(upper(btrim(COALESCE(p_codigo_proveedor, ''))), '');
  v_gtin text := NULLIF(regexp_replace(COALESCE(p_gtin, ''), '[^0-9]', '', 'g'), '');
  v_existing_id uuid;
BEGIN
  IF v_gtin IS NULL AND v_codigo IS NULL THEN
    RETURN;
  END IF;
  ...
  IF v_gtin IS NOT NULL THEN
    SELECT id
    INTO v_existing_id
    FROM public.productos_codigos_proveedor
  ...
  IF v_existing_id IS NULL THEN
    INSERT INTO public.productos_codigos_proveedor (
      empresa_id,
      proveedor_id,
      producto_id,
      codigo_proveedor,
      ...
      v_codigo,
      v_gtin,
      NULLIF(btrim(COALESCE(p_descripcion_proveedor, '')), ''),
      auth.uid(),
      NOW()
    );
  ELSE
    UPDATE public.productos_codigos_proveedor
    SET
      producto_id = p_producto_id,
      codigo_proveedor = COALESCE(v_codigo, codigo_proveedor),
      gtin = COALESCE(v_gtin, gtin),
      descripcion_proveedor = COALESCE(NULLIF(btrim(COALESCE(p_descripcion_proveedor, '')), ''), descripcion_proveedor),
      updated_at = NOW()
...
BEGIN
  ...
  LOOP
    ...
    IF v_linea->>'accion' = 'usar_existente' THEN
      ...
      UPDATE public.productos
      SET precio_compra = COALESCE(NULLIF(v_linea->>'precio_unitario', '')::numeric, precio_compra),
          updated_at = NOW()
      WHERE id = v_producto_id
        AND empresa_id = p_empresa_id;
    ELSE
...
CREATE OR REPLACE FUNCTION public.secure_importar_factura_electronica_compra(
  p_ejercicio_id uuid,
  p_proveedor_id uuid,
  p_bodega_id uuid,
  p_fecha date,
...
BEGIN
  ...
  RETURN public.importar_factura_electronica_compra(
    v_empresa_id,
    p_ejercicio_id,
    p_proveedor_id,
    p_bodega_id,
...
REVOKE ALL ON FUNCTION public.upsert_producto_codigo_proveedor(uuid, uuid, uuid, text, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.upsert_producto_codigo_proveedor(uuid, uuid, uuid, text, text, text) TO service_role;

REVOKE ALL ON FUNCTION public.importar_factura_electronica_compra(uuid, uuid, uuid, uuid, date, date, text, text, jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.importar_factura_electronica_compra(uuid, uuid, uuid, uuid, date, date, text, text, jsonb) TO service_role;

REVOKE ALL ON FUNCTION public.secure_importar_factura_electronica_compra(uuid, uuid, uuid, date, date, text, text, jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.secure_importar_factura_electronica_compra(uuid, uuid, uuid, date, date, text, text, jsonb) TO authenticated, service_role;
